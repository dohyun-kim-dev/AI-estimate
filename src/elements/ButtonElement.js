...
const ButtonElement = ({ variant = "primary", size = "medium", fullWidth = false, icon, iconPosition = "left", disabled = false, isRounded = false, bordered = false, layered = false, onClick, children, className, type = "button", borderRadius, backgroundColor, textColor, width, height, fontStyle, ...props }) => {
    return (_jsxs(StyledButton, { "$variant": variant, "$size": size, "$fullWidth": fullWidth, "$iconPosition": iconPosition, "$isRounded": isRounded, "$bordered": bordered, "$layered": layered, "$hasIcon": !!icon, "$borderRadius": borderRadius, "$backgroundColor": backgroundColor, "$textColor": textColor, "$width": width, "$height": height, "$fontStyle": fontStyle, disabled: disabled, onClick: onClick, className: className, type: type, ...props, children: [icon && iconPosition === "left" && _jsx(IconWrapper, { children: icon }), _jsx("span", { children: children }), icon && iconPosition === "right" && _jsx(IconWrapper, { children: icon })] }));
};
...
const getSizeStyles = (size, hasIcon) => {
    switch (size) {
        case "small":
            return css `

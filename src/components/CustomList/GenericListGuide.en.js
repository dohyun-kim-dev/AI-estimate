...
const GenericListGuide = () => {
    ...
    const columns = useMemo(() => [
        ...
        // 6-2. Date Formatting Column (Sortable)
        {
            ...
            sortable: true,
            formatter: (value) => dayjs(value).format("YYYY-MM-DD"), // Simple formatting using dayjs
        },
        // 6-3. Status Formatting Column (Using Styled Component, Sortable)
        {
            ...
            accessor: "status",
            sortable: true,
            formatter: (value) => _jsx(StatusBadge, { status: value, children: value.toUpperCase() }), // Use StatusBadge defined above
        },
        // 6-4. Number Formatting Column (Sortable)
        {
            ...
            accessor: "amount",
            sortable: true,
            formatter: (value) => `${value.toLocaleString()} KRW`, // Format number as currency
        },
    ...
    return (_jsx(GenericListUI, { 
        ...
        , columns: columns, fetchData: fetchData, excelFileName: "MockDataExport" // File name to be used for Excel download (extension excluded)
        , 
        // --- Optional Props ---
        ...
        renderHeaderActionButton: () => (_jsx("button", { onClick: () => alert("Additional action button clicked!"), style: { padding: "8px 16px", borderRadius: "0px", border: "none", background: "#214A72", color: "white" }, children: "Additional Action" })) }));
};

...
export const EstimateAccordion: React.FC<EstimateAccordionProps> = ({
  data,
  onItemClick,
}) => {
        ...
        <div key={index}>
          <EstimateAccordionItem
            ...
            items={category.sub_categories.map((sub) => ({
              ...
              price: sub.items
                .reduce(
                  (sum, item) => sum + parseInt(item.price.replace(/,/g, '')),
                  0
                )
                .toLocaleString(),
              description: '',
            ...
            onItemSelect={(itemId) =>
              handleSubItemSelect(category.category_name, itemId)
            }
          >
            {category.sub_categories.map((subCategory, subIndex) => (
              <EstimateAccordionItem
                ...
                depth={2}
                isSelected={
                  selectedSubItem === `${category.category_name}-${subIndex}`
                }
                onSelect={() =>
                  handleSubItemSelect(
